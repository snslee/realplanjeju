// blog-publish-detector v4.0 — B2+B3 통합 (mk_blog_slots 연결 + 실행로그 기록)
// 매일 11 KST / RSS 6채널 감지 + 노션 PUSH 체이닝 + 슬롯 동기화 + 대표 텔레그램 알림
...
// Levenshtein 유사도 (notion-publish-url-pusher v2 동일 로직)
...
async function parseRSS(url: string): Promise<{ title: string, link: string, pubDate: string }[]> {
  try {
    const r = await fetch(url, { headers: { 'User-Agent': 'realplan-blog-detector/4.0' } });
    if (!r.ok) return [];
...
// B2+B3: mk_blog_slots 매칭 → 티스토리포스트id + 실행로그 업데이트
async function syncToSlot(channelName: string, rssTitle: string, url: string): Promise<boolean> {
  try {
    ...
    for (const s of slots) {
      const sim = levSim(s.최종제목 || s.핵심키워드 || '', rssTitle);
      if (!best || sim > best.sim) best = { slot: s, sim };
    }
    if (!best || best.sim < 0.45) return false;

    // 중복 방지: 이미 같은 URL로 기록된 경우 skip
    const existingLog = Array.isArray(best.slot.실행로그) ? best.slot.실행로그 : [];
    if (existingLog.some((e: any) => e.url === url)) return false;
    ...
    // 티스토리 채널: 포스트 ID가 없을 때만 URL 저장
    if (isTistory && !best.slot.티스토리포스트id) {
      updateData.티스토리포스트id = url;
    }
    // pending 슬롯이 이제 발행됐으면 completed 처리
    ...
    if (error) { console.error('syncToSlot update err', error); return false; }
    return true;
  } catch (e) {
...
Deno.serve(async (_req) => {
  ...
  for (const ch of CHANNELS) {
    ...
    for (const it of items.slice(0, 10)) {
      ...
      // B2+B3: mk_blog_slots 동기화
      const matched = await syncToSlot(ch.name, it.title, it.link);
      if (matched) slotMatched++;
      newCount++;
  ...
  return new Response(JSON.stringify({ ok: true, totalNew, channelStats, version: 'v4.0' }), {
